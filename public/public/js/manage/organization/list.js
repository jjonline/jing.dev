$(function () {
    ...
    $('.table').on('change','.list-sort-input',function () {
        var id = $(this).data('id');
        var sort = $(this).val();
        utils.ajaxConfirm('确认修改该部门的排序？','/manage/organization/sort',{'id':id,'sort':sort},function () {
            location.reload();
        });
        //delete
    }).on('click','.delete',function () {
        var id = $(this).data('id');
        utils.ajaxConfirm('确认删除该部门么？','/manage/organization/delete',{'id':id},function () {
            location.reload();
        });
        // edit
            ...
            success: function (data) {
                if (data.error_code == 0) {
                    ...
                    utils.toast(data.error_msg, 3000, function () {
                        location.href = '/manage/organization/list';
                    });
                } else {
