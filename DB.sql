...
CREATE TABLE `com_department` (
  ...
  `level` int(11) NOT NULL COMMENT '部门层级：1->2->3逐次降低',
...
CREATE TABLE `com_user` (
  ...
  `is_leader` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否领导：1是0不是',
  `auth_code` char(32) NOT NULL COMMENT '授权code，用于cookie加密(可变)',
  `enable` tinyint(1) NOT NULL DEFAULT 1 COMMENT '启用禁用标记：1启用0禁用',
...
CREATE TABLE `com_staff` (
  ...
  `user_id` int(11) NOT NULL DEFAULT '' COMMENT '员工管理系统ID',
  `password` varchar(255) NOT NULL COMMENT '密码（密文）',
  `real_name` varchar(32) NOT NULL COMMENT '真实姓名',
  `gender` tinyint(1) NOT NULL DEFAULT -1 COMMENT '性别：-1未知0女1男',
  `mobile` varchar(20) NOT NULL DEFAULT '' COMMENT '手机号码',
