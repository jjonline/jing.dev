 ...
 * @date 2018-04-28 11:23
 * @file IpLocationService.php
 */
...
class IpLocationService
{
    ...
    public function __construct($filename = "qqwry.dat")
    {
        ...
        if (($this->fp = fopen($filename, 'rb')) !== false) {
            $this->first_ip = $this->getLong();
            $this->last_ip  = $this->getLong();
    ...
    private function getString($data = "")
    {
        ...
        return StringHelper::convert2utf8($data);
    }
    ...
    public function getLocation($ip = '')
    {
        ...
        $find_ip = $this->last_ip; // 如果没有找到就返回最后一条IP记录（QQWry.Dat的版本信息）
        while ($l <= $u) {
            // 当上边界小于下边界时，查找失败
            $i = floor(($l + $u) / 2); // 计算近似中间记录
            fseek($this->fp, $this->first_ip + $i * 7);
            $begin_ip = strrev(fread($this->fp, 4)); // 获取中间记录的开始IP地址
            // strrev函数在这里的作用是将little-endian的压缩IP地址转化为big-endian的格式
            // 以便用于比较，后面相同。
            if ($ip < $begin_ip) { // 用户的IP小于中间记录的开始IP地址时
                $u = $i - 1; // 将搜索的上边界修改为中间记录减一
            } else {
                fseek($this->fp, $this->getLong3());
                $end_ip = strrev(fread($this->fp, 4)); // 获取中间记录的结束IP地址
                if ($ip > $end_ip) { // 用户的IP大于中间记录的结束IP地址时
                    $l = $i + 1; // 将搜索的下边界修改为中间记录加一
                } else {
                    // 用户的IP在中间记录的IP范围内时
                    $find_ip = $this->first_ip + $i * 7;
                    break; // 则表示找到结果，退出循环
                }
            }
        }
        //获取查找到的IP地理位置信息
        fseek($this->fp, $find_ip);
